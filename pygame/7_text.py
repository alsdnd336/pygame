...
character_height = character_size[1] # 캐틱터의 세로 크기
...
enemy_height = enemy_size[1] # 캐틱터의 세로 크기
enemy_x_pos = (screen_width / 2) - (enemy_width / 2)  # 화면 가로의 절반 크기의 해당하는 곳에 위치
...
while running:
    ...
    for event in pygame.event.get(): # 어떤 이벤트가 발생했는지 체크
        ...
        if event.type == pygame.KEYDOWN:  # 키카 눌러졌는지 확인
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로 
                to_x -= character_speed 
            elif event.key == pygame.K_RIGHT: #캐릭터를 오른쪽으로
                to_x += character_speed
            elif event.key == pygame.K_UP: #캐릭턱를 위쪽으로
    ...
    character_x_pos += to_x * dt   # << 프레임에 따라 이동속도 조절해줌
    ...
    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255, 255, 255))  # << 시간을 그려준다.
    # 출력할 글자, True, 글자 색상
